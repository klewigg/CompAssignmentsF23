...
function Workshop() {
  ...
  const onSubmit = () => {
    ...
    const newFats = parseInt(fats);
    const newCarbs = parseInt(carbs);
    const newProtein = parseInt(protein);
    setRows([{ name:newName,
      calories:newCalories,
      fat:newFats,
      carbs:newCarbs,
      protein:newProtein}].concat(rows))
  }
  ...
  return (
    <div className={"workshop"}>
      <div className={"input-row"}>
        ...
        <TextField
          required
          label={"Dessert Protein"}
          value={protein}
          onChange={(event) => setProtein(event.target.value)}
        />
        <Button id="submit-entry" onClick={onSubmit}>Submit</Button>
